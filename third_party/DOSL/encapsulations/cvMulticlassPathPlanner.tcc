...
public:
    ...
    void find_paths (cv::Point start, cv::Point goal, int nPaths, bool vis=false) {
        ...
        //std::cout << "-- DOSL search " << nPaths << " path end in " << build_cost << "ms" << std::endl << std::endl;
        if (visualize) {
            cv::imshow("Display window", image_to_display);
            cv::waitKey(0);
        }
    ...
    // Constructor
    cvMulticlassPathPlanner ( cv::Mat obs_map,
                              freeNav::IS_OCCUPIED_FUNC<2> is_occupied,
                              int obsSizeThresh=0,
                              ...
                              cv_heuristic_type heuristic_type=heuristic_UNDEFINED )
                                    ...
                                    //,doslAlgorithm(is_occupied)
                                    ,doslAlgorithm::template Algorithm< cvMulticlassPathPlanner<doslAlgorithm,HClassType>, cvMPPnode<doslAlgorithm>, double>(is_occupied)
    {
    ...
    bool isEdgeAccessible (const cvMPPnode_& tn1, const cvMPPnode_& tn2) {
        // right nearby neighbor
        //std::cout << "tn1 (" << tn1.x << ", " << tn1.y << ")" << " / tn2 (" << tn2.x << ", " << tn2.y << ")" << std::endl;
