...
#include "../../../freeNav-base/basic_elements/point.h" // from RimJump
...
public:
    ...
    public:
        ...
        Algorithm (freeNav::IS_OCCUPIED_FUNC<2> is_occupied, AlgDerived* shared_instance_p = NULL);
        
        // =====================================================
        // Main search functions
        void search (double max_time_cost = 5e3);
        ...
        {
            //_dosl_default_fun_warn("ThetaStar::Algorithm::isSegmentFree"); // yz: remove warning, cause we add LineCrossObstacle
            //if(abs(n1.x - n2.x) > 1 || abs(n1.y - n2.y) > 1) { return false; }
            // use line-of-sight check from freeNav
            freeNav::Pointi<2> pt1 = {n1.x, n1.y}, pt2 = {n2.x, n2.y};
            if(freeNav::LineCrossObstacle(pt1, pt2, is_occupied_, neighbors_)) { return false; }
            //*c=(CostType)0.0;
            *c= sqrt(pow(n1.x - n2.x, 2.) + pow((n1.y - n2.y), 2.));
            return (true);
        }
    ...
    private:
        ...
        freeNav::IS_OCCUPIED_FUNC<2> is_occupied_; // from freeNav

        freeNav::Pointis<1> neighbors_; // from freeNav
...
ThetaStar::Algorithm<AlgDerived,NodeType,CostType>::Algorithm (freeNav::IS_OCCUPIED_FUNC<2> is_occupied,
                                                               AlgDerived* shared_instance_p) : _this (static_cast<AlgDerived*>(this))
{
    ...
    neighbors_ = GetNeightborOffsetGrids<1>();
}
...
template <class AlgDerived, class NodeType, class CostType>
void ThetaStar::Algorithm<AlgDerived,NodeType,CostType>::search (double max_time_cost )
{
    ...
    while (!(node_heap_p->empty()))
    {
        ...
        double cost_ms = (tv_after.tv_sec - tv_pre.tv_sec)*1e3 + (tv_after.tv_usec - tv_pre.tv_usec)/1e3;
        if(cost_ms >= max_time_cost) {
            std::cout << "DOSL::ThetaStar reach " << max_time_cost << "ms" << std::endl;
            break;
        }
        expand_count++;
//        if(expand_count >= 3000) {
//            std::cout << "DOSL::ThetaStar expand_count >= 5000 " << std::endl;
//            break;
//        }
        ...
        for (auto it = thisNodeInHash_p->successors.begin(); it!=thisNodeInHash_p->successors.end(); ++it)
        {
            ...
            // yz: thisNodeInHash_p->came_from == null cause error
            if ( thisNodeInHash_p->came_from && _this->isSegmentFree (*(thisNodeInHash_p->came_from), *this_neighbour_node_in_hash_p, &grandparentSegmentCost)) { // path 2
