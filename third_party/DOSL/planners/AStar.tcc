...
AStar::Algorithm<AlgDerived,NodeType,CostType>::Algorithm (freeNav::IS_OCCUPIED_FUNC<2> is_occupied, AlgDerived* shared_instance_p) : _this (static_cast<AlgDerived*>(this))
{
...
template <class AlgDerived, class NodeType, class CostType>
void AStar::Algorithm<AlgDerived,NodeType,CostType>::search (double max_time_cost)
{
    ...
    while (!(node_heap_p->empty()))
    {
        ...
        if(cost_ms >= max_time_cost) {
            std::cout << "DOSL::AStar reach " << max_time_cost << "ms" << std::endl;
            break;
        }
        expand_count++;
//        if(expand_count >= 3000) {
//            std::cout << "DOSL::AStar expand_count >= 5000 " << std::endl;
//            break;
//        }
