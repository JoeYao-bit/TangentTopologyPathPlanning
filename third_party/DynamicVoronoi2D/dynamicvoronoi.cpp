...
void DynamicVoronoi::initializeEmpty(int _sizeX, int _sizeY, bool initGridMap) {
  ...
  if (initGridMap) {
    if (allocatedGridMap && !gridMap.empty()) {
      gridMap.clear();
      allocatedGridMap = false;
    }
  }
  ...
  if (initGridMap) {
    ...
    for(int i=0; i<sizeX; i++) {
        std::vector<bool> row(sizeY, false);
...
void DynamicVoronoi::initializeMap(int _sizeX, int _sizeY, const std::vector<std::vector<bool> >&  _gridMap) {
  gridMap = _gridMap;
