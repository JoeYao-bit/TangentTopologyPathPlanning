...
void LineOfSightScanner::computeAllDirNeighbours(ScannerStacks& data, int sx, int sy) const {
    ...
    // generate the first up interval and down interval
 ...
 *    :         :      XXX|         |XXX   XXX|         |XXX
 */
void LineOfSightScanner::generateTautDirectionStartingStates(ScannerStacks& data, int sx, int sy) const {
    ...
    if (!topRightOfBlocked || !bottomRightOfBlocked) {
        ...
        int y = sy;
        if (!(grid.topRightOfBlockedTile(x, y) && grid.bottomRightOfBlockedTile(x, y))) {
...
void LineOfSightScanner::generateAllDirectionStartingStates(ScannerStacks& data, int sx, int sy) const {
    ...
    if (!bottomLeftOfBlocked || !bottomRightOfBlocked) {
        ...
        //std::cout << "up: left extent : " << leftExtent.toFloat() << " right extent: " << rightExtent.toFloat() << std::endl;
        generateUpwards(data, leftExtent, rightExtent, sx, sy, sy, true, true);
    }
    ...
    if (!topLeftOfBlocked || !topRightOfBlocked) {
        ...
        //std::cout << "down: left extent : " << leftExtent.toFloat() << " right extent: " << rightExtent.toFloat() << std::endl;
        generateDownwards(data, leftExtent, rightExtent, sx, sy, sy, true, true);
    }
...
void LineOfSightScanner::exploreStates(ScannerStacks& data, int sx, int sy) const {
    while (data.intervalStack.size() > 0) {
        ...
        if (currState.y > sy) {
            ...
            Fraction leftProjection = (currState.xL - sx).multiplyDivide(dy+1, dy) + sx;

            int leftBound = leftUpExtent(currState.xL.ceil(), currState.y);
            if (currState.xL.isWholeNumber() && grid.bottomRightOfBlockedTile(currState.xL.n, currState.y)) leftBound = currState.xL.n;
            
            if (leftProjection < leftBound) { // leftProjection < leftBound
                leftProjection = Fraction(leftBound);
                leftInclusive = true;
            }

            // (Px-Bx)*(Py-By+1)/(Py-By) + Bx
            Fraction rightProjection = (currState.xR - sx).multiplyDivide(dy+1, dy) + sx;
            
            int rightBound = rightUpExtent(currState.xR.floor(), currState.y);
            if (currState.xR.isWholeNumber() && grid.bottomLeftOfBlockedTile(currState.xR.n, currState.y)) rightBound = currState.xR.n;

            if (rightProjection > rightBound) { // rightBound < rightProjection
            ...
            if (leftInclusive && rightInclusive) {
                if (leftProjection <= rightProjection) {
                    generateUpwards(data, leftProjection, rightProjection, sx, sy, currState.y, true, true);
                }
            }
            else if (leftProjection < rightProjection) {
                generateUpwards(data, leftProjection, rightProjection, sx, sy, currState.y, leftInclusive, rightInclusive);
            }
        }
        else {
            // Upwards
            ...
            if (currState.xL.isWholeNumber() && grid.topRightOfBlockedTile(currState.xL.n, currState.y)) leftBound = currState.xL.n;
            ...
            if (currState.xR.isWholeNumber() && grid.topLeftOfBlockedTile(currState.xR.n, currState.y)) rightBound = currState.xR.n;
            ...
            if (leftInclusive && rightInclusive) {
                if (leftProjection <= rightProjection) {
                    generateDownwards(data, leftProjection, rightProjection, sx, sy, currState.y, true, true);
                }
            }
            else if (leftProjection < rightProjection) {
                generateDownwards(data, leftProjection, rightProjection, sx, sy, currState.y, leftInclusive, rightInclusive);
            }
        }
    }
}
...
void LineOfSightScanner::generateUpwards(ScannerStacks& data, Rational leftBound, Rational rightBound, int sx, int sy, int currY, bool leftInclusive, bool rightInclusive) const {
    //std::cout << __FUNCTION__  << " start " << std::endl;
    generateAndSplitIntervals(data,
            currY + 2, currY + 1,
            sx, sy,
...
void LineOfSightScanner::generateDownwards(ScannerStacks& data, Rational leftBound, Rational rightBound, int sx, int sy, int currY, bool leftInclusive, bool rightInclusive) const {
    //std::cout << __FUNCTION__  << " start " << std::endl;
    generateAndSplitIntervals(data,
            currY - 1, currY - 1,
            sx, sy,
...
//yz: TODO: visualize interval after generateAndSplitIntervals

 // yz: detect all intervals that may contain collision free pt
 // scan from left to right, until reach max right extend ration
void LineOfSightScanner::generateAndSplitIntervals(ScannerStacks& data, int checkY, int newY, int sx, int sy, Rational leftBound, Rational rightBound, bool leftInclusive, bool rightInclusive) const {
    Rational left = leftBound;
    int leftFloor = left.floor();

    // Up: !bottomRightOfBlockedTile && bottomLeftOfBlockedTile
    if (leftInclusive && left.isWholeNumber() && !grid.isBlocked(leftFloor-1, checkY-1) && grid.isBlocked(leftFloor, checkY-1)) {
    ...
    while(true) {
        ...
        if (!grid.isBlocked(right-1, checkY-1)) {
            data.stackPush(ScanInterval(newY, left, Rational(right), leftInclusive, true));
        }
    ...
    if (!grid.isBlocked(leftFloor, checkY-1)) {
        data.stackPush(ScanInterval(newY, left, rightBound, leftInclusive, rightInclusive));
    } else {
        // The possibility of there being one degenerate interval at the end. ( !bottomLeftOfBlockedTile(xR, checkY) )
        if (rightInclusive && rightBound.isWholeNumber() && !grid.isBlocked(rightBound.n, checkY-1)) {
