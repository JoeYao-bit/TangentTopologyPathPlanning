...
struct ScannerStacks {
    ...
    std::vector<ScanInterval> intervalStack_backup;

    inline void clear() {
        neighbours.clear();
        intervalStack.clear();
        intervalStack_backup.clear();
    }

    // yz: add visible neighbors to visible start
    inline void addSuccessor(GridVertex gridVertex) {
        neighbours.push_back(gridVertex);
    }

    inline void stackPush(ScanInterval interval) {
        intervalStack_backup.push_back(interval);
...
public:
    ...
    void computeExtents(); // yz: compute the four direction expand distant

    // yz: which point can current point move to
...
//private:
    ...
    std::vector<int> rightDownExtents; // yz: each grid's most right down occupied boundary x coordinate
    std::vector<int> leftDownExtents; // yz: each grid's most left down occupied boundary x coordinate
