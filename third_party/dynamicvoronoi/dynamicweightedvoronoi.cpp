...
DynamicVoronoiRRT::DynamicVoronoiRRT() {
  sqrt2 = sqrt(2.0);
  data = NULL;
  type_weight_ = 0;
  weighting_map_ = NULL;
...
void DynamicVoronoiRRT::initializeEmpty(int _sizeX, int _sizeY, bool initGridMap) {
  if (data) {
    for (int x=0; x<sizeX; x++) delete[] data[x];
    delete[] data;
  }
...
void DynamicVoronoiRRT::initializeWeightingMap(int _sizeX, int _sizeY, int** _weightingMap) {
...
void DynamicVoronoiRRT::initializeHumanInfo(int _sizeX, int _sizeY, std::vector<humanCell> hinfo){
...
void DynamicVoronoiRRT::initializeMap(int _sizeX, int _sizeY, const std::vector<std::vector<bool> >&  _gridMap) {
    gridMap = _gridMap;
...
void DynamicVoronoiRRT::occupyCell(int x, int y) {
  gridMap[x][y] = 1;
  setObstacle(x,y);
}
void DynamicVoronoiRRT::clearCell(int x, int y) {
  gridMap[x][y] = 0;
  removeObstacle(x,y);
}

void DynamicVoronoiRRT::setObstacle(int x, int y) {
  dataCell c = data[x][y];
...
void DynamicVoronoiRRT::removeObstacle(int x, int y) {
  dataCell c = data[x][y];
...
void DynamicVoronoiRRT::exchangeObstacles(std::vector<INTPOINT>& points) {
...
// yz: update voronoi graph
void DynamicVoronoiRRT::update(bool updateRealDist) {
  // remove old obstacle and add new obstacles
...
void DynamicVoronoiRRT::setTypeWeighting(int t){
...
void DynamicVoronoiRRT::settingGains(double social, int border, double direction_gain){
...
int DynamicVoronoiRRT::computeDistance(int xa, int ya, int xb, int yb){
...
/// ============================================================================================
double  DynamicVoronoiRRT::costFromline(int x0, int y0, int y1, int x1){
...
float DynamicVoronoiRRT::getDistance( int x, int y ) {
  if( (x>0) && (x<sizeX) && (y>0) && (y<sizeY)) return data[x][y].dist;
  else return -INFINITY;
}

float DynamicVoronoiRRT::getHumanDistance(int x, int y){

  return (sqrt(dist_closest_humans_[x][y]));

}

float DynamicVoronoiRRT::getGridDistance( int x, int y ) {
  if( (x>0) && (x<sizeX) && (y>0) && (y<sizeY)) return data[x][y].gridDist;
  else return -INFINITY;
}
bool DynamicVoronoiRRT::isVoronoi( int x, int y ) {
  dataCell c = data[x][y];
  return (c.voronoi==free || c.voronoi==voronoiKeep);
}

bool DynamicVoronoiRRT::isVoronoiAlternative(int x, int y) {
  int v = alternativeDiagram[x][y];
  return (v == free || v == voronoiKeep);
}

// yz: add new obstacle and remove old obstacles
void DynamicVoronoiRRT::commitAndColorize(bool updateRealDist) {
  // ADD NEW OBSTACLES
  for (unsigned int i=0; i<addList.size(); i++) {
    ...
    // yz: if current grid not in queue
...
void DynamicVoronoiRRT::checkVoro(int x, int y, int nx, int ny, dataCell& c, dataCell& nc) {
...
void DynamicVoronoiRRT::reviveVoroNeighbors(int &x, int &y) {
  for (int dx=-1; dx<=1; dx++) {
    int nx = x+dx;
    if (nx<=0 || nx>=sizeX-1) continue;
    for (int dy=-1; dy<=1; dy++) {
...
bool DynamicVoronoiRRT::isOccupied(int x, int y) {
  dataCell c = data[x][y];
  return (c.obstX==x && c.obstY==y);
}

bool DynamicVoronoiRRT::isOccupied(int &x, int &y, dataCell &c) {
  return (c.obstX==x && c.obstY==y);
}

void DynamicVoronoiRRT::visualize(const char *filename) {
  // write ppm files
...
void DynamicVoronoiRRT::updateAlternativePrunedDiagramWithHumans(){
...
void DynamicVoronoiRRT::updateAlternativePrunedDiagram() {
...
bool DynamicVoronoiRRT::markerMatchAlternative(int x, int y) {
...
int DynamicVoronoiRRT::getNumVoronoiNeighborsAlternative(int x, int y) {
  int count = 0;
  for (int dx = -1; dx <= 1; dx++) {
    for (int dy = -1; dy <= 1; dy++) {
...
DynamicVoronoiRRT::markerMatchResult DynamicVoronoiRRT::markerMatch(int x, int y) {
