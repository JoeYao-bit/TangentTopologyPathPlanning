#ifndef _DYNAMICWEIGHTEDVORONOI_H_
#define _DYNAMICWEIGHTEDVORONOI_H_
...
#include "../DynamicVoronoi2D/bucketedqueue.h"

//! A DynamicVoronoi object computes and updates a distance map and Voronoi diagram.
class DynamicVoronoiRRT {
...
public:
  ...
  //! Initialization with an empty mapb
  void initializeEmpty(int _sizeX, int _sizeY, bool initGridMap=true);
  //! Initialization with a given binary map (false==free, true==occupied)
  void initializeMap(int _sizeX, int _sizeY, const std::vector<std::vector<bool> >& _gridMap);
...
private:
  struct dataCell {
    ...
    char voronoi;
    char queueing;
    int obstX; // yz: closest obstacle to current grid
    int obstY; // yz: closest obstacle to current grid
    bool needsRaise;
    int sqdist;
    float gridDist;
  };
